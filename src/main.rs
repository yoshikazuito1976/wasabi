...
fn efi_main(image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    let vw = vram.width;
    let vh = vram.height;
    fill_rect(&mut vram, 0x000000, 0, 0, vw, vh).expect("fill_rect_failed");
    ...
    writeln!(
        w,
        "Total: {total_memory_pages}
        pages = {total_memory_size_mib} MiB"
    )
    .unwrap();
...
fn draw_test_pattern<T: Bitmap>(buf: &mut T) {
    ...
    for (i, c) in colors.iter().enumerate() {
        let y = i as i64 * h;
        fill_rect(buf, *c, left, y, h, h).expect("fill_rect_failed");
        fill_rect(buf, !*c, left + h, y, h, h).expect("fill_rect_failed");
    }
